...
{	
	...
	public void run()
	{
		while(true)
		{
			...
			int min;
			min = Math.min(Math.min(w1, w2), w3);
			...
			if (System.nanoTime() >= nextSendOK)
			{
				 ...
				 * If there is send packet, remove it from queue.
				...
				{	
					...
					if (Quantum1 >= packet.getLength()) {
						Quantum1 -= packet.getLength();
				...
				{	
					...
					if (Quantum2 > packet.getLength()) {
						Quantum2 -= packet.getLength();
						sendPacket(packet, startTime);
				...
				if ((packet = buffers[2].peek()) != null)
				{
					
					if (Quantum3 > packet.getLength()) {
						Quantum3 -= packet.getLength();
						sendPacket(packet, startTime);
...
//				addQ = Math.max(Math.max(addQ1, addQ2), addQ2);
